...
def vanity_projects
  # List the title of all movies in which the director also
  # appeared as the starring actor. Show the movie id and title and director's name.

  # Note: Directors appear in the 'actors' table.
  Movie.joins(:actors).where('director_id = actors.id AND castings.ord = 1').select('movies.id, movies.title, actors.name')
end
...
#  Recorded: List the year in which NONE of the movies had a rating above 8.

  # # groups specifies a group attribute
...
def golden_age
  ...
  Movie.select('avg(score) as avg_score, yr / 10 * 10 AS decade').group('decade').order('avg_score DESC').first.decade
...
def cast_list(title)
  ...
  Movie.find_by(title: title).actors.order('castings.ord') #find_by

end

def costars(name)
  # List the names of the actors that the named actor has ever appeared with.
  Actor.find_by(name: name).movies.joins(:actors).where('name != ?', name).select('actors.name').distinct.pluck(:name)
end
...
def longest_career
  ...
  Actor.select(:name, :id, 'max(movies.yr) - min(movies.yr) as career').joins(:movies).order('career DESC').group(:name, :id).first
end
